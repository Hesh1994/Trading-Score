...
def calculate_sma(df, period_short=15, period_long=45):
    ...
    sma_short = ta.sma(df['close'], length=period_short)
    sma_long = ta.sma(df['close'], length=period_long)
...
def calculate_stochastic(df, k_period=14, d_period=3, smooth_k=3):
    ...
    # stoch returns DataFrame with columns like STOCHk_..., STOCHd_...
    # Extract the first set of columns (should be %K and %D)
    stoch_k = stoch.iloc[:, 0]  # First column is %K
    stoch_d = stoch.iloc[:, 1]  # Second column is %D
    
    return stoch_k.bfill().ffill(), \
           stoch_d.bfill().ffill()
...
def calculate_aroon(df, period=25):
    ...
    # aroon returns DataFrame with AroonUp and AroonDown columns
    aroon_up = aroon.iloc[:, 0]    # First column is Aroon Up
    aroon_down = aroon.iloc[:, 1]  # Second column is Aroon Down
...
def calculate_bollinger(df, period=20, std_dev=2.0):
    ...
    # bb returns DataFrame with BBL_, BBM_, BBU_ columns
    bb_lower = bb.iloc[:, 0]   # Lower band
    bb_middle = bb.iloc[:, 1]  # Middle band (SMA)
    bb_upper = bb.iloc[:, 2]   # Upper band
    
    return bb_upper.bfill(), \
           bb_middle.bfill(), \
           bb_lower.bfill()
...
def evaluate_aroon_criteria(aroon_up, aroon_down, buy_criteria, sell_criteria):
    ...
    if pd.notna(aroon_up) and pd.notna(aroon_down):
        # Buy: AroonUp > 70 AND AroonDown < 30
        ...
        # Sell: AroonDown > 70 AND AroonUp < 30
...
def score_stock(df, ticker, config=None, global_config=None):
    ...
    result = {
        ...
        'buy_score': 0.0,
        'sell_score': 0.0,
        'signals': {},  # Store which indicators triggered
    }
